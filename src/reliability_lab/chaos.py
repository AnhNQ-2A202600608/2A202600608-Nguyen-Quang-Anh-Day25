...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    for breaker in gateway.breakers.values():
        ...
        for log in breaker.transition_log:
            if log["to"] == "open":
                last_open_ts = float(log["ts"])
            elif log["to"] == "closed":
                if last_open_ts is not None:
                    delta = (float(log["ts"]) - last_open_ts) * 1000.0
                    deltas.append(delta)
                    last_open_ts = None
    if not deltas:
        return None
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig | None = None) -> RunMetrics:
    ...
    6. Return metrics
    """
    overrides = scenario.provider_overrides if scenario is not None else None
    gateway = build_gateway(config, overrides)
    metrics = RunMetrics()
    num_requests = config.load_test.requests
    ...
    concurrent_enabled = getattr(config.load_test, "concurrent", False)
    if concurrent_enabled:
        max_workers = getattr(config.load_test, "max_workers", 10)
